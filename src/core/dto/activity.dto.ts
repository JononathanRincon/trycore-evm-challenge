...
export const CreateActivitySchema = z.object({
  name: z.string().min(1, 'El nombre de la actividad es obligatorio').max(150),
  bac: z.number().min(0, 'El BAC (presupuesto planificado) no puede ser negativo'),
  plannedProgress: z
    .number()
  ...
  actualCost: z.number().min(0, 'El costo real (AC) no puede ser negativo'),
});
