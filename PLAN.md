...
### Casos borde obligatorios en tests (Fase 3) — confirmados y con aserciones exactas

1. **Ejemplo numérico testigo**: BAC=10,000, 50%/40%, AC=6,000 → CPI=0.6667, SPI=0.8, EAC=15,000, VAC=−5,000.
2. **AC = 0 con EV = 0** → CPI=`null`, interpretación "Sin costos registrados", EAC=`null`.
3. **AC = 0 con EV > 0** → CPI=`null`, interpretación "Sin costos registrados", EAC=`null`.
4. **AC > 0 con EV = 0** → CPI=`0` (número, **no** `null`, porque el denominador sí es >0), CV=`−AC`, EAC=`null` (protección de división por cero cuando CPI=0), VAC=`null`, interpretación "Sobre presupuesto (sobrecosto)".
5. **PV = 0 con EV = 0** → SPI=`null`, interpretación "Sin avance planificado".
6. **PV = 0 con EV > 0** → SPI=`null`, interpretación "Sin avance planificado", SV=EV (positivo, consistente).
7. **Proyecto sin actividades (`[]`)** → retorno seguro en 0/`null` sin excepciones.

> Nota de proceso: el caso 4 (AC>0, EV=0 → CPI=0 explícito) fue una omisión inicial del plan, detectada en revisión antes del merge del PR #1 (motor de cálculo EVM). Documentado como parte del control de calidad del proceso asistido por IA en `AI_PROCESS.md`.
